...
const headLessons: Record<TransformerPresetId, [string, string]> = {
  ...
  return <>
    ...
    <div className="attention-layout">
      <section className="panel attention-matrix-panel">
        ...
        <div className="attention-controls"><label><span className="attention-control-label">Trace</span><select aria-label="Attention teaching trace" value={stored.preset} onChange={event => { setStored(current => ({ ...current, preset: event.target.value as TransformerPresetId })); setQuery(null); }}>{transformerPresets.map(preset => <option key={preset.id} value={preset.id}>{preset.label}</option>)}</select></label><label><span className="attention-control-label attention-head-label">Head <InfoTooltip label="Explain attention heads"><div className="learning-popover"><h3>What is a head?</h3><p>A head is one small context-finding pattern. It gives every allowed earlier token a share, then makes a weighted mix of their information.</p><p><strong>Do not treat one head as the model’s final opinion.</strong> A layer combines all heads, then later layers transform the result.</p><p>In <strong>keys to the cabinet are</strong>, Head 1 favors <strong>keys</strong> for grammar while Head 2 favors nearby <strong>cabinet</strong>. Together, both ideas remain available.</p><p>In <strong>Maya gave Lina her notebook</strong>, one head favors <strong>Lina</strong> and another keeps <strong>Maya</strong> in play. The final <strong>notebook</strong> row also attends to <strong>her</strong>, which carries the unresolved link.</p></div></InfoTooltip></span><select aria-label="Attention head" value={stored.head} onChange={event => setStored(current => ({ ...current, head: Number(event.target.value) }))}><option value={0}>Head 1</option><option value={1}>Head 2</option></select></label></div>
